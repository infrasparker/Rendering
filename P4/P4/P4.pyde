...
def green_mountain():
    ...
    gridSize = 20
    bounds = (-100, 100)
    tileSize = (bounds[1] - bounds[0]) / gridSize
    
    for y in range(0, gridSize):
        for x in range(0, gridSize):
            beginShape()
            texture(mountainTexture)
            vertex(bounds[0] + tileSize * x, bounds[0] + tileSize * y, 0,
                   (1.0 / gridSize) * x, (1.0 / gridSize) * y) # bl
            vertex(bounds[0] + tileSize * (x + 1), bounds[0] + tileSize * y, 0,
                   (1.0 / gridSize) * (x + 1), (1.0 / gridSize) * y) # br
            vertex(bounds[0] + tileSize * (x + 1), bounds[0] + tileSize * (y + 1), 0, 
                   (1.0 / gridSize) * (x + 1), (1.0 / gridSize) * (y + 1)) # tr
            vertex(bounds[0] + tileSize * x, bounds[0] + tileSize * (y + 1), 0,
                   (1.0 / gridSize) * x, (1.0 / gridSize) * (y + 1)) # tl
            endShape()
    popMatrix()
